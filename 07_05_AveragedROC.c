		...
		//load management mode
			...
			//going from stable to unstable
				...
				//timer hasn't started
					...
					printf("Shed value in ms= %d", sheddingCounter);
					if(sheddingCounter > maxTimeShed) {
						maxTimeShed = sheddingCounter;
					}
					if(sheddingCounter < minTimeShed) {
						minTimeShed = sheddingCounter;
					}
					//put average
					*/
...
void describeLEDS(int describeBehaviour) {
	...
	if(describeBehaviour == 1) {
		...
		//--here you check if your switch is on, check if led is off, compare with associated green led
        	...
        	//CURRENTLY FAILING SINCE WE ARE PASSIGN -VE VALUE IN hob()
